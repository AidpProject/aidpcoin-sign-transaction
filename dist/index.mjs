...
function $c3f6c693698dc7cd$export$c5552dfdbc7cec71(network, rawTransactionHex, UTXOs, privateKeys) {
    const networkMapper = {
        aidp: (0, $hCgyA$chains).aidp.main,
        "aidp-test": (0, $hCgyA$chains).aidp.test,
    ...
    //@ts-ignore
    const AIDPCOIN = (0, $hCgyA$toBitcoinJS)(coin);
    const tx = $hCgyA$Transaction.fromHex(rawTransactionHex);
    const txb = $hCgyA$TransactionBuilder.fromTransaction(tx, AIDPCOIN);
    function getKeyPairByAddress(address) {
        const wif = privateKeys[address];
        const keyPair = $hCgyA$ECPair.fromWIF(wif, AIDPCOIN);
        return keyPair;
    }
